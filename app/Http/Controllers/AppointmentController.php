...
class AppointmentController extends Controller
{
    ...
    public function getSlots(Request $request, $lawyerId): JsonResponse
    {
        ...
        // Use map keyed by canonical 24h time to avoid duplicates from overlapping availability rows
        $slotMap = [];
        ...
        // Sort by canonical time and return the display values
        ksort($slotMap);
        $slots = array_values($slotMap);

        if ($request->boolean('debug')) {
            Log::info('getSlots debug', [
                'lawyer_id' => $lawyerId,
                'date' => $request->date,
        ...
        return new JsonResponse([
            ...
            // Optional debug to inspect availability issues during testing
    ...
    public function book(Request $request): JsonResponse
    {
        ...
        // Check for conflicts
        // Only block slots that are already scheduled/confirmed; allow retry if a prior pending booking never completed
