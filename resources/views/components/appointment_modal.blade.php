        ...
        <div class="modal-content">
            ...
            <form id="appointment-form">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-semibold">{{ __('messages.time') }}</label>
                ...
                <div class="modal-footer">
                    ...
                    <button class="btn btn-primary" type="submit" id="book-btn">{{ __('messages.book') }}</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    // Ensure modal script initializes only once, even if component is included multiple times
    if (!window.__appointmentModalInitialized) {
        window.__appointmentModalInitialized = true;

        const initAppointmentModal = function() {
            ...
            const fetchSlots = async () => {
                ...
                try {
                    const res = await fetch(`/lawyers/${lawyerId}/slots?date=${date}&debug=1`);
                    const data = await res.json();

                    console.log('Slots response', data);

                    if (!res.ok) {
                        const msg = data.message || 'Unable to load slots';
                        timeSlotsContainer.innerHTML = `<div class="text-danger small text-center" style="grid-column: 1 / -1;">${msg}</div>`;
                        return;
                    }
            ...
            window.openAppointmentModal = function(lawyerId) {
                ...
                // Reset form but restore previously selected date if any
                form.reset();
                if (dateInput) {
                ...
                appointmentModal.addEventListener('show.bs.modal', function(event) {
                    ...
                    if (button) {
                        ...
                        if (lawyerId) {
                            ...
                            // Reset time selection only (keep chosen date)
                            timeInput.value = '';
                            timeSlotsContainer.innerHTML =
                                '<div class="text-muted small text-center" style="grid-column: 1 / -1;">Select a date to view available times</div>';

                            // If a date is already chosen, fetch slots immediately
                            if (dateInput) {
                                if (!dateInput.value) {
