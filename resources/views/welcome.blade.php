...
@section('content')
            ...
            <div class="row align-items-center">
                <div class="col-lg-6 mb-5 mb-lg-0">
                    <span class="badge bg-primary bg-opacity-10 text-primary px-3 py-2 rounded-pill mb-3 fw-semibold">
                        <i class="bi bi-stars me-1"></i> {{ __('messages.hero_badge') }}
                    </span>
                    <h1 class="display-4 fw-bold mb-4 lh-tight">
                        {{ __('messages.hero_title_1') }} <span class="text-primary">{{ __('messages.hero_title_2') }}</span>
                    </h1>
                    ...
                    <div class="d-flex flex-column flex-sm-row gap-3">
                        @guest
                            ...
                            <a href="{{ route('register') }}" class="btn btn-outline-primary btn-lg px-4 py-3 fw-semibold">
                        ...
                        @else
                            ...
                            @if (Auth::user()->role === 'lawyer')
                                <a href="{{ route('lawyer.dashboard') }}" class="btn btn-accent btn-lg px-4 py-3 fw-semibold">
                                    <i class="bi bi-speedometer2 me-2"></i>{{ __('messages.dashboard') }}
                                </a>
                            @elseif(Auth::user()->role === 'admin')
                                <a href="{{ route('admin.dashboard') }}" class="btn btn-accent btn-lg px-4 py-3 fw-semibold">
                        ...
                        <form action="{{ route('ai.ask') }}" method="POST" id="aiQuestionForm">
                            ...
                            <div class="mb-3">
                                <textarea name="question" class="form-control bg-light border-0 p-3" rows="4"
                                    placeholder="{{ __('messages.ai_input_placeholder') }}" style="resize: none;" required></textarea>
                            </div>
            ...
            <div class="row g-0">
                ...
                <div class="col-md-4">
                    <div class="stat-card">
                        ...
                        <div class="text-muted fw-semibold">{{ __('messages.stat_client_satisfaction') }}</div>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="stat-card">
                        <div class="stat-number">24/7</div>
                        <div class="text-muted fw-semibold">{{ __('messages.stat_ai_support') }}</div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <section class="py-6 bg-light-section">
        <div class="container py-5">
            <div class="text-center mb-5">
                <h2 class="fw-bold display-6 mb-3">{{ __('messages.why_choose_title') }}</h2>
    ...
    <section class="py-6 bg-white">
        <div class="container py-5">
            <div class="row align-items-center">
                ...
                <div class="col-lg-5 offset-lg-1">
                    <h2 class="fw-bold display-6 mb-5">{{ __('messages.how_it_works_title') }}</h2>
    ...
    <section class="py-5 bg-primary text-white">
        <div class="container text-center py-4">
            <h2 class="fw-bold mb-3">{{ __('messages.cta_title') }}</h2>
            <p class="lead mb-4 text-white-50">{{ __('messages.cta_desc') }}</p>
            @guest
                <a href="{{ route('register') }}"
                    class="btn btn-accent btn-lg px-5 fw-bold">{{ __('messages.cta_btn_guest') }}</a>
            @else
                <a href="{{ route('lawyers.index') }}"
                    class="btn btn-accent btn-lg px-5 fw-bold">{{ __('messages.cta_btn_user') }}</a>
            @endguest
        </div>
    </section>
@endsection
...
@push('scripts')
    <script>
        document.getElementById('aiQuestionForm').addEventListener('submit', async function(e) {
            ...
            responseAlert.textContent = '{{ __('messages.analyzing_question') }}';
            ...
            try {
                ...
                if (response.ok && data.ok) {
                    ...
                    responseAlert.innerHTML =
                        '<strong>{{ __('messages.ai_response_label') }}</strong><br><div style="white-space: pre-wrap;" class="mt-2">' +
                        answer + '</div>';
                } else {
