        ...
        <div class="container">
            <div class="row align-items-center">
                <div class="col-lg-7">
                    ...
                    @else
                        <div class="mt-4 d-flex gap-2">
                            <a href="{{ route('lawyers.index') }}" class="btn btn-lg btn-primary me-2">Browse Lawyers</a>
                            @if (Auth::user()->role === 'lawyer')
                                <a href="{{ route('lawyer.dashboard') }}" class="btn btn-lg btn-accent">My Dashboard</a>
                            @elseif(Auth::user()->role === 'admin')
                                <a href="{{ route('admin.dashboard') }}" class="btn btn-lg btn-accent">Admin Panel</a>
                            @else
                                <a href="{{ route('appointments.index') }}" class="btn btn-lg btn-accent">My Appointments</a>
                            @endif
                        </div>
                    @endguest
                </div>
                <div class="col-lg-5 d-none d-lg-block">
                    <div class="card card-ghost p-4">
                        ...
                        <p class="small-muted mb-3">Ask legal questions in natural language and get concise summaries and next steps.</p>
                        
                        <form action="{{ route('ai.ask') }}" method="POST" id="aiQuestionForm">
                            @csrf
                            <div class="mb-3">
                                <textarea name="question" class="form-control" rows="3" placeholder="e.g., What are the requirements for filing a divorce in Bangladesh?" required></textarea>
                            </div>
...
document.getElementById('aiQuestionForm').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (response.ok && data.ok) {
            ...
            responseAlert.innerHTML = '<strong>Answer:</strong><br><div style="white-space: pre-wrap;">' + answer + '</div>';
        } else {
            responseAlert.className = 'alert alert-danger';
            responseAlert.textContent = data.error || data.message || 'An error occurred. Please try again.';
        }
    } catch (error) {
        responseAlert.className = 'alert alert-danger';
