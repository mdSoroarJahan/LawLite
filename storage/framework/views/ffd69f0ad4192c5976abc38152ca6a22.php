...
<?php $__env->startSection('content'); ?>
    <div class="container py-4">
        <h1 class="mb-4"><?php echo e(__('messages.notifications')); ?></h1>

        <?php if($notifications->isEmpty()): ?>
            <div class="alert alert-info">
                <p class="mb-0"><?php echo e(__('messages.no_notifications_yet')); ?></p>
            </div>
        <?php else: ?>
                    ...
                    <div class="list-group-item <?php echo e($notification->read_at ? '' : 'list-group-item-primary'); ?>">
                        <div class="d-flex w-100 justify-content-between">
                            <h6 class="mb-1">
                                <?php if(!$notification->read_at): ?>
                                    <span class="badge bg-primary me-2"><?php echo e(__('messages.new')); ?></span>
                                <?php endif; ?>
                                <?php echo e($notification->data['title'] ?? __('messages.notification')); ?>

                            </h6>
                            <small class="text-muted"><?php echo e($notification->created_at->diffForHumans()); ?></small>
                        </div>
                        <p class="mb-1"><?php echo e($notification->data['message'] ?? __('messages.new_notification_message')); ?>
                        ...
                        <?php if(!$notification->read_at): ?>
                            <form method="POST" action="<?php echo e(route('notifications.mark-read', $notification->id)); ?>"
                                ...
                                <?php echo csrf_field(); ?>
                                <button type="submit"
                                    class="btn btn-sm btn-link p-0"><?php echo e(__('messages.mark_as_read')); ?></button>
                            </form>
                        <?php endif; ?>
                    </div>
            ...
            <?php if($notifications->where('read_at', null)->count() > 0): ?>
                <div class="mt-3">
                    <form method="POST" action="<?php echo e(route('notifications.mark-all-read')); ?>">
                        <?php echo csrf_field(); ?>
                        <button type="submit"
                            class="btn btn-sm btn-secondary"><?php echo e(__('messages.mark_all_as_read')); ?></button>
                    </form>
                </div>
            <?php endif; ?>
        <?php endif; ?>
